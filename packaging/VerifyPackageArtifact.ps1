...
function Get-ProjectProperty {
    ...
    if ($null -eq $node) {
        throw "Icod.DirTree.csproj does not declare '$Name'."
    }
...
try {
    ...
    if ('dirtree' -ne "$($commands[0].Name)" -or 'dotnet' -ne "$($commands[0].Runner)") {
        throw "Package '$($package.Name)' does not declare the expected dirtree/dotnet tool command."
    }
...
Write-Host "Exact Icod.DirTree package verification completed successfully for $packageVersion ($Configuration)."
